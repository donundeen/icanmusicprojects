...
const int UDPPort = 7005; // the UDP port that Max is listening on

// NETWORK+SENSOR CONFIGS
const char *DEVICE_ID = "/l3g4200/val";
L3G gyro;
...
void device_loop() {
  ...
  sendOSCUDP((int)gyro.g.x, (int)gyro.g.y, (int)gyro.g.z );
  delay(10);
}
...
void sendOSCUDP(int gyrox, int gyroy, int gyroz ){ /* egs
 ...
 if(WiFi.status() == WL_CONNECTED){   
  ...
  oscmsg.add(gyrox).add(gyroy).add(gyroz).add(ipbuffer);
